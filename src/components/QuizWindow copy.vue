<template>
  <div id="quiz_outer_wrap">
    ...
    <!-- <div class="titleTooltip">
      <svg-title-tooltip-icon />
      <div class="tooltiptext">
        <p>스킨명은 2022년 출시 기준으로 작성되었습니다.</p>
        <p>그 전 출시 스킨은 정답으로 처리되지 않습니다.</p>
      </div>
    </div> -->
      ...
      <div class="quiz_inner_wrap">
        ...
        <div class="confirm_wrap">
          <div class="my_pick_wrap">
            <div class="my_pick_title">나의 예측</div>
...
// import ViewerRatingDisabledService from '@/scripts/service/viewer-rating-disabled-service'
...
// import SvgTitleTooltipIcon from '@/components/svgs/SvgTitleTooltipIcon'
...
export default {
  ...
  computed: {
    ...
    guageRate() {
      return Math.floor((this.liveTime / this.startTime) * 100 - 2)
    },
    convertLiveSecondToMinute() {
      let min = parseInt((this.times % 3600) / 60)
      let min2 = min.toString().padStart(2, '0')
      let sec = this.times % 60
  ...
  methods: {
    ...
    handleClickCommitMyQuizPick() {
      if (store.getters.getMyPick && store.getters.getTimes > 0) {
        this.$store.commit(types.COMMIT_MY_QUIZ_PICK)
      }
    },
